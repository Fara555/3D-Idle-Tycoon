...
public class BuildingView : MonoBehaviour
{
    ...
    [SerializeField] private GameObject gainRoot; 
    [SerializeField] private TMP_Text gainText;
    [SerializeField] private int gainMaxDigits = 3; 
    ...
    {
        _isWorking = true;
        if (_lastProgress >= 0f && progressBar)
    ...
    private void HandleYield(int amount)
    {
        ...
        if (_resourceAnimation == null)
            _resourceAnimation = gainRoot.GetComponent<ResourceUIAnimation>();

        if (!gainRoot.activeSelf)
        {
            gainRoot.SetActive(true); 
        }
        else
        {
